...
TZEND
  ...
  context "from example iCal file" do
    before(:each) do
      @it = RiCal.parse_string <<ENDCAL
BEGIN:VCALENDAR
...
ENDCAL
    ...
    it "should provide start time" do
      event = @it[0].events[0]
      event.dtstart.should be_kind_of(RiCal::PropertyValue::DateTime) || be_kind_of(RiCal::PropertyValue::Date)
    end
  end
